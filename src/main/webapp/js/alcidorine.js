...
var alcidorine = angular.module('alcidorineApp', [])
	...
	.factory("IsOnMobileDevice", ['$log', function($log){
		var check = false;
	...
	.factory("StringUtil", ['$log', function($log){
		return {
	...
	.controller('NavigationCtrl', ['$scope', '$log', '$window', '$location', 'IsOnMobileDevice', 'ROOT_URL', 'API_PREFIX', function($scope, $log, $window, $location, IsOnMobileDevice, ROOT_URL, API_PREFIX){
		...
		$scope.resizeListener = function() {
			if($window.innerWidth < 650 || IsOnMobileDevice) {
				$scope.global.menuClass = "small_menu";
				$scope.global.isOnMobileDevice = true;
			} else {
				$scope.global.menuClass = "large_menu";
				$scope.global.isOnMobileDevice = false;
			}
		}
	...
	.controller('ArticleCtrl', ['$scope', '$log', '$sce', '$window', 'ROOT_URL', 'StringUtil', function($scope, $log, $sce, $window, ROOT_URL, StringUtil) {
		...
		$scope.getArticles = function() {
			gapi.client.alcidorine.alcidorine.articles.list().execute(function(resp) {
				$scope.$apply($scope.fetchArticles(resp.items));
			})
		}
		
		$scope.view = function(articleId, articleTitle) {
			$window.location.href = ROOT_URL + "/articles/" + articleId + "/" + StringUtil.formatTextForUrlRewrite(articleTitle);
		}
